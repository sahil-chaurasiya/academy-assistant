...
export default function StudentsPage() {
  ...
  // Load today's already-checked-in students on mount
  useEffect(() => {
    api.get('/visits/today').then(r => {
      const ids = new Set<string>(
        r.data.map((v: { student?: { _id?: string } }) => v.student?._id).filter(Boolean)
      );
      setCheckedInIds(ids);
    });
  ...
  const toggleAll = () =>
    setSelected(selected.size === students.length ? new Set() : new Set(students.map(s => s._id)));

  const clearFilters = () => {
    setSearch(''); setActiveTags([]); setLevel('');
    setRatingMin(''); setRatingMax(''); setJoinedFrom(''); setJoinedTo('');
  };
  ...
  const handleCheckIn = async (id: string) => {
    ...
    // error: do nothing, button stays enabled so they can retry
  };
  ...
  const handleBulk = async () => {
    ...
    } catch (err) { setBulkMessage((err as ErrShape)?.response?.data?.message || 'Action failed'); }
    ...
    <div className="p-4 md:p-8 max-w-5xl">
      ...
      <div className="flex items-center justify-between mb-5">
        <div>
          ...
          <p className="text-muted text-sm mt-0.5">{students.length} enrolled</p>
        </div>
        <Link href="/students/new" className="btn-primary whitespace-nowrap">+ Add Student</Link>
      </div>

      {/* Search + filter toggle */}
      <div className="flex gap-2 mb-3 flex-wrap">
        <input type="text" value={search} onChange={e => setSearch(e.target.value)}
          placeholder="Search name or phone…" className="input flex-1 max-w-xs" />
        <button onClick={() => setShowFilters(f => !f)}
          className={`btn-secondary text-xs px-3 whitespace-nowrap ${hasActiveFilters ? 'border-amber text-amber' : ''}`}>
          {showFilters ? '▲ Filters' : '▼ Filters'}{hasActiveFilters ? ' ●' : ''}
        </button>
        {hasActiveFilters && (
          <button onClick={clearFilters} className="text-xs text-rose/70 hover:text-rose transition-colors px-2">Clear all</button>
        )}
      </div>
        ...
        <div className="card p-4 mb-5 space-y-4">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            ...
            <div>
              ...
              <input type="number" min={1} max={10} className="input" placeholder="1"
                value={ratingMin} onChange={e => setRatingMin(e.target.value)} />
            </div>
            <div>
              <label className="label">Rating max</label>
              <input type="number" min={1} max={10} className="input" placeholder="10"
                value={ratingMax} onChange={e => setRatingMax(e.target.value)} />
            </div>
          </div>
          {/* Date range */}
          ...
          {/* Tag filter */}
          <div>
            <label className="label">Filter by weakness</label>
                ...
                return (
                  <button key={tag._id} onClick={() => toggleTag(tag.name)}
                    className={`px-3 py-1 rounded-full text-xs font-semibold border transition-all capitalize ${
                      isActive ? 'bg-ink text-cream border-ink' : `${tag.color} hover:scale-105`
                    }`}>
                    {isActive && '✕ '}{tag.name}
                  </button>
                );
              })}
              {!tags.length && <span className="text-xs text-muted italic">No tags yet — add in Settings</span>}
            </div>
          </div>
        </div>
      )}

      {/* Bulk action bar — appears when any student is selected */}
      {selected.size > 0 && (
        <div className="card p-3 mb-4 bg-ink/5 border-ink/20 flex flex-wrap items-center gap-3">
          <span className="text-sm font-semibold text-ink">{selected.size} selected</span>
          <select className="input w-auto text-xs py-1.5" value={bulkAction}
          ...
          {bulkAction === 'assignTopic' && (
            <select className="input w-auto text-xs py-1.5" value={bulkTopicId} onChange={e => setBulkTopicId(e.target.value)}>
              ...
              {tags.map(tag => {
                ...
                return (
                  <button key={tag._id} type="button"
                    ...
                    className={`px-2.5 py-1 rounded-full text-xs font-semibold border capitalize transition-all ${on ? tag.colorSelected : tag.color}`}>
                    {tag.name}
                  </button>
                );
              })}
          ...
          <button onClick={handleBulk} disabled={bulkRunning || !bulkAction}
            className={`btn-primary text-xs px-3 py-1.5 ${bulkAction === 'delete' ? 'bg-rose hover:bg-rose/90' : ''}`}>
            {bulkRunning ? '…' : 'Apply'}
          </button>
          <button onClick={() => { setSelected(new Set()); setBulkAction(''); setBulkMessage(''); }}
            className="text-xs text-muted hover:text-ink transition-colors">Deselect all</button>
          {bulkMessage && <span className="text-xs text-emerald font-medium">{bulkMessage}</span>}
        </div>
      )}
      ...
      {loading ? (
        ...
        <div className="card p-12 text-center">
          ...
          {!hasActiveFilters && <Link href="/students/new" className="text-amber text-sm hover:underline mt-2 block">Add your first student →</Link>}
        </div>
      ) : (
        <div className="grid gap-3">
          ...
          {students.map(s => (
            <div key={s._id} className={`card p-4 md:p-5 hover:shadow-card-hover transition-shadow ${selected.has(s._id) ? 'ring-2 ring-amber/40' : ''}`}>
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap">
                    <Link href={`/students/${s._id}`} className="font-semibold text-ink hover:text-amber transition-colors">
                      {s.name}
                    </Link>
                  ...
                  {s.weaknessTags?.length > 0 && (
                    <div className="flex flex-wrap gap-1 mt-1.5">
                      {s.weaknessTags.map(tagName => {
                        ...
                        return (
                          <span key={tagName} className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium border capitalize ${tagDef?.color || 'bg-parchment text-muted border-parchment'}`}>
                            {tagName}
                          </span>
                        );
                      })}
                ...
                {/* Actions */}
                <div className="flex flex-col md:flex-row items-end md:items-center gap-1.5 flex-shrink-0">
                  <span className="text-muted text-xs hidden md:block">{format(new Date(s.joiningDate), 'MMM yyyy')}</span>
                  <button
                    onClick={() => handleCheckIn(s._id)}
                    disabled={checkedInIds.has(s._id)}
                    className={`text-xs px-2.5 py-1.5 whitespace-nowrap rounded-lg border font-semibold transition-colors ${
                  ...
                  <Link href={`/students/${s._id}/edit`} className="btn-secondary text-xs px-2.5 py-1.5">Edit</Link>
                  <button onClick={() => handleDelete(s._id, s.name)} disabled={deletingId === s._id}
                    className="text-rose/60 hover:text-rose text-xs px-2 py-1.5 transition-colors">
